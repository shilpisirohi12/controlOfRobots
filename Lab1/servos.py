...
class servos:
    ...
    def calibrateSpeed(self):
        ...
        for i in range(41):
            ...
            time.sleep(5)
            spd = self.getSpeeds()
            #print("(",round(leftWheel,2),"  ", round(rightWheel,2), "): ", round((self.getSpeeds()[0]+self.getSpeeds()[1])/2,2))
            arrLeft.append(round(leftWheel,2))
            #arrRight.append(round(rightWheel,2))
            arrSpeedL.append(round(spd[0],2))
            arrSpeedR.append(round(spd[1],2))
            self.lSpeed.append(round(spd[0],2))
            self.rSpeed.append(round(spd[1],2))
