...
def rotateRight(maze):
    ...
    while(float(tick_count) <= num_tick):
        ...
        time.sleep(0.1)
        tick_count = servos.rTick
        print("Right Tick count:", tick_count)
    time.sleep(0.1)
    servos.setSpeedRPS(1.5,1.5)
...
def rotateLeft(maze):
    global orient
    servos.resetCounts()
    print("Rotating Left, current orientation:" , orient)
    orient += 1;
    orient = orient %4;
    wheel_diam = 2.5
    dist = 3.14
    ...
    tick_length = (float(wheel_diam)*3.14)/32
    num_tick = float(dist) / float(tick_length)
    tick_count = servos.lTick
        
    while(float(tick_count) <= num_tick-2):
        servos.setSpeedsRPS(-0.4, 0.4)
        time.sleep(0.1)
        tick_count = servos.lTick
        #print("TIck count:", tick_count)
